...
An AI agent skill for stress-testing technical plans before they harden into implementation.
...
For other agents, copy the contents of `SKILL.md` into your agent's reusable instructions, rules, memory, prompt library, or skill system. The format is intentionally plain Markdown so it can be adapted to Claude, Cursor, Windsurf, Cline, Aider, OpenCode, custom agents, and similar tools.
...
If your agent does not support `$skill` invocation, say:

```text
Use the grill-my-plan instructions to stress-test this architecture plan against my codebase and outside engineering evidence.
```
